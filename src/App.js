...
export const MyLocation = createContext(null);
...
const App = () => {
  ...
  useEffect(() => {
    GetLocationView()
      .then((locations) => {
        ...
        const allCategories = [
          ...new Set(locations.map((curEle) => curEle.attributes.category)),
        ];
  ...
  return (
    <>
      <MyLocation.Provider
        value={{ category, places, allPlaces, currentCategory, filterData }}
      >
        <Nav />
        <Routes>
